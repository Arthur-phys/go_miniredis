...
func init() {
	...
	flag.StringVar(&strCache, "cache_type", "Cache", "Type of cache to use. Available caches are:\n - 'Cache': Has a simple interface and sub-optimal alogrithms. It mostly served as a placeholder for tests in this library. Nonetheless, it can be useful if you plan to use redigo for a small app with not too much traffic\n - 'Cache': A cache made to be used by a large application that allows multiple goroutines to access the dictionary at the same time granted the key looked for is in different shards.")
	...
	flag.Uint64Var(&workerAmount, "worker_amount", 10, "Number of workers to initialize. Every worker will be able to attend a connection independently until this connection is closed or the keepAlive parameter is exceeded.")
