...
var (
	...
	UnexpectedFirstByte            = Error{"First byte was different from expected", 4, nil, make(map[string]string)}
	...
	InsufficientLength             = Error{"Insufficient length for command", 8, nil, make(map[string]string)}
	UnableToConvertIndexToInt      = Error{"Unable to convert the provided index to ain integer", 9, nil, make(map[string]string)}
	FunctionNotFound               = Error{"Function provided not found for current implementation", 9, nil, make(map[string]string)}
)
